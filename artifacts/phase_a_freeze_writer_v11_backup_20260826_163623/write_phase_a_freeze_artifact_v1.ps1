...
try {
    ...
    if (
        (Get-FileHash $FinalJsonPath -Algorithm SHA256).Hash
        -ne $JsonHash
    ) {
    ...
    if (
        (Get-FileHash $FinalTextPath -Algorithm SHA256).Hash
        -ne $TextHash
    ) {
    ...
    if (
        (Get-FileHash $FinalManifestPath -Algorithm SHA256).Hash
        -ne $ManifestHash
    ) {
    ...
    if (
        $Moved
        -and (Test-Path -LiteralPath $FinalDir)
    ) {
