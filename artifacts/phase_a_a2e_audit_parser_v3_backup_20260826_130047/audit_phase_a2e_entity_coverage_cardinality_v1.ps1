...
def looks_negative_value(value):
    ...
    if isinstance(value, str):
        return value.strip().casefold() in NONE_TOKENS

    return False


def maybe_multi_string(value):
    ...
    code_like = re.compile(
        r"^[A-Za-z]{1,8}[-_]?\d{1,5}$"
    )

    if (
        len(parts) >= 2
        and all(
            code_like.fullmatch(item)
            for item in parts
        )
    ):
        return parts
...
def add_positive(
    ...
    positive.append(
        {
            "key": key,
            "value": maybe_multi_string(value),
        }
    )
...
def consume_object(
    ...
    if isinstance(obj, dict):
        ...
        for key, value in obj.items():
            ...
            elif looks_negative_value(value):
                add_negative(
                    negative,
                    key,
                )
...
if len(expected_keys) != 20:
    raise RuntimeError(
        f"expected entity key count={len(expected_keys)}, expected 20; "
