...
def test_single_callsite_order_five_inputs_two_dependencies_and_three_bindings():
    ...
    cp11 = named_calls("run_cp11_presentation_stage")
...
def test_cp11_consumes_exact_post_stage_state_and_wrapper_chain_is_unchanged():
    cp11 = named_calls("run_cp11_presentation_stage")[0]
    assert [arg.id for arg in cp11.args] == [
    ...
    assert [(keyword.arg, keyword.value.id) for keyword in cp11.keywords] == [
