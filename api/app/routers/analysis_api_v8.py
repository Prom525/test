...
def summarize_band_question(
    ...
    opmerkingen = [
        ...
        if (r.get("opmerking_raw") or "").strip()
    ][:10]

    vervangingen = [
        r for r in recent_rows
        if r.get("vervangen") is True
    ][:10]
...
def summarize_location_question(
    ...
    actieadvies = []
    if summary.get("n_vervuiling", 0) > 0:
...
def summarize_scraper_question(
    ...
    return {
        ...
        "resultaat": rows[0],
        "trend_patronen": [
            f"{len(rows)} locatie-selecties gevonden.",
        ],
        "relevante_opmerkingen": [],
        "vervangingen": [],
        "ongewone_slijtage": None,
...
def band_deep_analysis(
    ...
    vervangingen = [
        r for r in lifecycle.get("resultaat", [])
        if r.get("replace_event")
    ][:10]
    ...
    return {
        ...
        "trend_patronen": trend_patronen[:10],
