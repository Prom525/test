...
def is_target_excel_source(source_file: str) -> bool:
    ...
    # alleen onder Tata steel root
    if ROOT_PATH.lower() not in sf:
        return False

    # alleen excelbestanden
    if not sf.endswith((".xls", ".xlsx", ".xlsm")):
        return False

    # totaal.xlsx overslaan
    if os.path.basename(sf) == "totaal.xlsx":
        return False

    # minimaal één mapdeel dat begint met "lijsten"
    parts = [p.lower() for p in PureWindowsPath(source_file).parts]
    if not any(part.startswith("lijsten") for part in parts):
        return False
...
def main():
    with engine.begin() as conn:
        ...
        for source_file, raw_sheet in target_sheets:
            ...
            if inspection:
                lijn_code = inspection[1] or infer_lijn_code_from_source(source_file)
            else:
                # fallback als header er wel conceptueel is maar nog niet goed gelabeld werd
                lijn_code = infer_lijn_code_from_source(source_file)

                exists_source_sheet = conn.execute(text("""
                    SELECT 1
                    FROM sb_inspections_v0
                    WHERE source_file = :source_file
                      AND sheet = :sheet
